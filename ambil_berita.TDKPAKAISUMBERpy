...
# --- FUNGSI: BIKIN NAMA MEDIA DARI URL --- DIHAPUS KARENA GAK DIPAKE LAGI
...
def rewrite_with_groq(title, link, media):
    ...
    if not GROQ_API_KEY or not konten_asli or len(konten_asli) < 150:
        return format_ke_html(f"Badan redaksi PULNEW.com sedang melakukan pemutakhiran data."), soup, title, ""
    ...
    try:
        ...
        print(f"Asli: {len(konten_asli)} char -> Rewrite: {len(hasil_json.get('isi',''))} char")
...
<meta name="description" content="{article.get('lead','')}">
...
</html>""" # <--- BARIS "Sumber:" SUDAH DIHAPUS
        ...
        try:
            ...
            for item in soup.find_all('item')[:3]:
                ...
                berita_data = {
                    ...
                    "image": img_url,
                    "body": body
                    # "source_name" dan "source_url" SUDAH DIHAPUS DARI SINI
                }
